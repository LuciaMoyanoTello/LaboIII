...
public class HumanRoundPlayer extends RoundPlayer {
    ...
    private Integer getCardToPlay() {
        ...
        for (int i = 0; i < this.getHandCards().size(); i++) {
            LetterByLetterPrinter.println(i + " - " + this.getHandCards().get(i));
        }
        ...
        do {
            try {
                ...
                if (selectedCardIndex < 0 || selectedCardIndex >= this.getHandCards().size()) {
        ...
        } while (selectedCardIndex < 0 || selectedCardIndex >= this.getHandCards().size());
