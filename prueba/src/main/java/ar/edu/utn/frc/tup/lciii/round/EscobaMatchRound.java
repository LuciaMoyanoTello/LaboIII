...
public class EscobaMatchRound {
    ...
    private void solveSevenWinner() {
        ...
        if (humanSevens > appSevens) {
            LetterByLetterPrinter.println("El jugador gana 1 punto por tener más sietes.");
            roundPlayerHuman.setRoundPoints(roundPlayerHuman.getRoundPoints() + 1);
            if (humanSevens == 4) {
                LetterByLetterPrinter.println("El jugador gana 1 punto extra por tener los cuatro sietes.");
                roundPlayerHuman.setRoundPoints(roundPlayerHuman.getRoundPoints() + 1);
            }
        } else if (appSevens > humanSevens) {
            LetterByLetterPrinter.println("La app gana 1 punto por tener más sietes.");
            roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            if (appSevens == 4) {
                LetterByLetterPrinter.println("La app gana 1 punto extra por tener los cuatro sietes.");
                roundPlayerApp.setRoundPoints(roundPlayerApp.getRoundPoints() + 1);
            }
        } else {
            LetterByLetterPrinter.println("No hay puntos por cantidad de sietes. Ambos jugadores tienen la misma cantidad.");
        }
    }
    ...
    private Boolean sevenOroCheck(List<Card> cards) {
        for (Card card : cards) {
            if (card.getNumber().equals(7) && card.getCardSuit().equals(CardSuit.OROS)) {
                return true;
            }
        }
