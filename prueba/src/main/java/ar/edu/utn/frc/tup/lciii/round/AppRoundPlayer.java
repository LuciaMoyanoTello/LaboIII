...
public class AppRoundPlayer extends RoundPlayer {
    ...
    private List<Card> getCardsWithSeven(List<List<Card>> subconjuntos) {
        ...
        for (List<Card> subset : subconjuntos) {
            long countSevens = subset.stream().filter(card -> card.getNumber() == 7).count();
            if (countSevens > maxSevens) {
                maxSevens = (int) countSevens;
                bestSubset = subset;
            }
        }
    ...
    private List<Card> getCardsWithMoreOros(List<List<Card>> subconjuntos) {
        ...
        for (List<Card> subset : subconjuntos) {
            long countOros = subset.stream().filter(card -> card.getCardSuit() == CardSuit.OROS).count();
            if (countOros > maxOros) {
                maxOros = (int) countOros;
                bestSubset = subset;
            }
        }
